...
public:
    ...
    uint16_t GetPduVersion()
    {
        return m_pdu_version;
    }
    uint32_t GetClientType()
    {
    ...
    void SetOnlineStatus(uint32_t status)
    {
        m_online_status = status;
    }
    uint32_t GetOnlineStatus()
    {
...
private:
    ...
    void _HandleClientAddUserRequest(CImPdu* pPdu);
    void _HandleClientDelUserRequest(CImPdu* pPdu);
    void _HandleClientAgreeAddUserRequest(CImPdu* pPdu);
    void _HandleClientListAddUserRequest(CImPdu* pPdu);
    void _HandleClientQueryUserRequest(CImPdu* pPdu);
private:
