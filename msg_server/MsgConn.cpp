        ...
        void* pParam)
{
    ...
    if (cur_time > g_last_stat_tick + LOG_MSG_STAT_INTERVAL)
    {
        ...
        log(
                "up_msg_cnt=%u, up_msg_miss_cnt=%u, down_msg_cnt=%u, down_msg_miss_cnt=%u ",
...
void CMsgConn::SendUserStatusUpdate(uint32_t user_status)
{
    ...
    CImUser* pImUser = CImUserManager::GetInstance()->GetImUserById(
            GetUserId());
    if (!pImUser)
    {
        return;
...
void CMsgConn::Close(bool kick_user)
{
    ...
    pImUser = CImUserManager::GetInstance()->GetImUserByLoginName(
            GetLoginName());
    if (pImUser)
    {
        pImUser->DelUnValidateMsgConn(this);
...
void CMsgConn::OnTimer(uint64_t curr_tick)
{
    ...
    if (CHECK_CLIENT_TYPE_MOBILE(GetClientType()))
    {
        ...
        {
            log("mobile client timeout, handle=%d, uid=%u ", m_handle,
                    GetUserId());
            Close();
            return;
        }
            ...
            it != m_send_msg_list.end();)
    {
        ...
        {
            log("!!!a msg missed, msg_id=%u, %u->%u ", msg.msg_id, msg.from_id,
                    GetUserId());
            g_down_msg_miss_cnt++;
...
void CMsgConn::HandlePdu(CImPdu* pPdu)
{
    ...
    case CID_OTHER_HEARTBEAT: //心跳
        ...
        break;
    case CID_LOGIN_REQ_USERLOGIN: //登录
        _HandleLoginRequest(pPdu);
        break;
    case CID_LOGIN_REQ_LOGINOUT: //登出
        _HandleLoginOutRequest(pPdu);
        break;
    case CID_LOGIN_REQ_DEVICETOKEN: //登录设备标识
        _HandleClientDeviceToken(pPdu);
        break;
    case CID_LOGIN_REQ_KICKPCCLIENT: //提出PC端用户
    ...
    case CID_FILE_DEL_OFFLINE_REQ:
        ...
        break;
    case CID_BUDDY_LIST_ADD_USER_REQUEST:
        _HandleClientAddUserRequest(pPdu);
        break;
    case CID_BUDDY_LIST_AGREE_ADD_USER_REQUEST:
        _HandleClientAgreeAddUserRequest(pPdu);
        break;
...
void CMsgConn::_HandleLoginRequest(CImPdu* pPdu)
{
    ...
    CDbAttachData attach_data(ATTACH_TYPE_HANDLE, m_handle, 0); //用于标识本端接受到数据库响应的标识
    ...
    pdu.SetSeqNum(pPdu->GetSeqNum());
    pDbConn->SendPdu(&pdu); //发送到数据库校验登录
}

void CMsgConn::_HandleLoginOutRequest(CImPdu *pPdu)
{
    log("HandleLoginOutRequest, user_id=%d, client_type=%u. ", GetUserId(),
            GetClientType());
...
void CMsgConn::_HandleKickPCClient(CImPdu *pPdu)
{
    ...
    if (!CHECK_CLIENT_TYPE_MOBILE(GetClientType())) //必须从手机登录
    {
        log(
...
void CMsgConn::_HandleClientRecentContactSessionRequest(CImPdu *pPdu)
{
    ...
    CHECK_PB_PARSE_MSG(
            msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    log(
            "HandleClientRecentContactSessionRequest, user_id=%u, latest_update_time=%u. ",
...
void CMsgConn::_HandleClientMsgData(CImPdu* pPdu)
{
    ...
    if (msg.from_user_id()
            == msg.to_session_id() && CHECK_MSG_TYPE_SINGLE(msg.msg_type()))
    {
...
void CMsgConn::_HandleClientGetMsgListRequest(CImPdu *pPdu)
{
    ...
    log(
            "HandleClientGetMsgListRequest, req_id=%u, session_type=%u, session_id=%u, msg_id_begin=%u, msg_cnt=%u. ",
...
void CMsgConn::_HandleClientGetMsgByMsgIdRequest(CImPdu *pPdu)
{
    ...
    log(
            "_HandleClientGetMsgByMsgIdRequest, req_id=%u, session_type=%u, session_id=%u, msg_cnt=%u.",
...
void CMsgConn::_HandleClientMsgReadAck(CImPdu* pPdu)
{
    ...
    uint32_t msg_id = msg.msg_id();
    log(
            "HandleClientMsgReadAck, user_id=%u, session_id=%u, msg_id=%u, session_type=%u. ",
...
void CMsgConn::_HandleClientGetLatestMsgIDReq(CImPdu *pPdu)
{
    ...
    uint32_t session_id = msg.session_id();
    log(
            "HandleClientGetMsgListRequest, user_id=%u, session_id=%u, session_type=%u. ",
...
void CMsgConn::_HandleClientP2PCmdMsg(CImPdu* pPdu)
{
    ...
    log("HandleClientP2PCmdMsg, %u->%u, cmd_msg: %s ", from_user_id, to_user_id,
            cmd_msg.c_str());
...
void CMsgConn::_HandleClientUserInfoRequest(CImPdu* pPdu)
{
    ...
    log("HandleClientUserInfoReq, req_id=%u, user_cnt=%u ", GetUserId(),
            user_cnt);
    CDBServConn* pDBConn = get_db_serv_conn_for_login();
    if (pDBConn)
    {
...
void CMsgConn::_HandleClientAllUserRequest(CImPdu* pPdu)
{
    ...
    log("HandleClientAllUserReq, user_id=%u, latest_update_time=%u. ",
...
void CMsgConn::_HandleClientUsersStatusRequest(CImPdu* pPdu)
{
    ...
    log("HandleClientUsersStatusReq, user_id=%u, query_count=%u.", GetUserId(),
            user_count);
...
void CMsgConn::_HandleClientDepartmentRequest(CImPdu *pPdu)
{
    ...
    CHECK_PB_PARSE_MSG(
            msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    log("HandleClientDepartmentRequest, user_id=%u, latest_update_time=%u.",
            GetUserId(), msg.latest_update_time());
    CDBServConn* pDBConn = get_db_serv_conn();
    if (pDBConn)
    {
...
void CMsgConn::_HandleClientDeviceToken(CImPdu *pPdu)
{
    if (!CHECK_CLIENT_TYPE_MOBILE(GetClientType()))
    {
        log("HandleClientDeviceToken, user_id=%u, not mobile client.",
                GetUserId());
        return;
    }
    IM::Login::IMDeviceTokenReq msg;
    ...
    log("HandleClientDeviceToken, user_id=%u, device_token=%s ", GetUserId(),
            device_token.c_str());
...
void CMsgConn::_HandleClientAddUserRequest(CImPdu* pPdu)
{
    ...
    log("_HandleClientAddUserRequest, user_id=%u, peer_id=%u. ", GetUserId(),
            peer_id);

    CImUser* pToImUser = CImUserManager::GetInstance()->GetImUserById(peer_id);
    int status = 0;
    if (pToImUser)
    {
        log("pToImUser is on line ");
        CImPdu* pdu = new CImPdu();
        IM::Buddy::IMAddUserReq msg3;
        msg3.set_peer_id(GetUserId());
        pdu->SetPBMsg(&msg3);
        pdu->SetServiceId(IM::BaseDefine::SID_BUDDY_LIST);
        pdu->SetCommandId(IM::BaseDefine::CID_BUDDY_LIST_ADD_USER_NOTIFY);
        pToImUser->BroadcastPdu(pdu, NULL);
        status = 1; //yi jing fa song
    } else
    {
        log("pToImUser is off line ");
    }
    CDBServConn* pConn = get_db_serv_conn();
    if (pConn)
    {
...
void CMsgConn::_HandleClientDelUserRequest(CImPdu* pPdu)
{
    IM::Buddy::IMDelUserReq msg;
    log("_HandleClientDelUserRequest, user_id=%u. ", GetUserId());
    CHECK_PB_PARSE_MSG(
            msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    uint32_t peer_id = msg.peer_id();
    log("_HandleClientDelUserRequest, user_id=%u, peer_id=%u. ", GetUserId(),
            peer_id);
    ...
    if (pToImUser)
    {
        log("pToImUser is on line ");
        CImPdu* pdu = new CImPdu();
        IM::Buddy::IMDelByUser msg3;
        msg3.set_peer_id(peer_id);
        pdu->SetPBMsg(&msg3);
        pPdu->SetServiceId(IM::BaseDefine::SID_BUDDY_LIST);
        pPdu->SetCommandId(IM::BaseDefine::CID_BUDDY_LIST_DEL_USER_NOTIFY);
        pToImUser->BroadcastPdu(pdu, NULL);
    } else
        log("pToImUser is off line ");
    CDBServConn* pConn = get_db_serv_conn();
...
void CMsgConn::_HandleClientAgreeAddUserRequest(CImPdu* pPdu)
{
    IM::Buddy::IMAgreeAddUserReq msg;
    log("_HandleClientAgreeAddUserRequest, user_id=%u ", GetUserId());
    CHECK_PB_PARSE_MSG(
            msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    uint32_t peer_id = msg.peer_id();
    ...
    if (pToImUser)
    {
        ...
        pPdu->SetServiceId(IM::BaseDefine::SID_BUDDY_LIST);
        pPdu->SetCommandId(
                IM::BaseDefine::CID_BUDDY_LIST_AGREE_ADD_USER_NOTIFY);
        pToImUser->BroadcastPdu(pdu, NULL);
    }
    log("pToImUser is off line ");
    CDBServConn* pConn = get_db_serv_conn();
...
void CMsgConn::_HandleClientListAddUserRequest(CImPdu* pPdu)
{
...
void CMsgConn::_HandlePushShieldRequest(CImPdu* pPdu)
{
    ...
    CHECK_PB_PARSE_MSG(
            msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    log("_HandlePushShieldRequest, user_id=%u, shield_status ", GetUserId(),
            msg.shield_status());
    CDBServConn* pDBConn = get_db_serv_conn();
    if (pDBConn)
    {
