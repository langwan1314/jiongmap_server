...
log	("MsgServer max files can open: %d ", getdtablesize());
	...
	serv_info_t* db_server_list = read_server_config(&config_file, "DBServerIP",
			"DBServerPort", db_server_count);

	uint32_t login_server_count = 0;
	serv_info_t* login_server_list = read_server_config(&config_file,
			"LoginServerIP", "LoginServerPort", login_server_count);

	uint32_t route_server_count = 0;
	serv_info_t* route_server_list = read_server_config(&config_file,
			"RouteServerIP", "RouteServerPort", route_server_count);

	uint32_t push_server_count = 0;
	serv_info_t* push_server_list = read_server_config(&config_file,
			"PushServerIP", "PushServerPort", push_server_count);

	uint32_t file_server_count = 0;
	serv_info_t* file_server_list = read_server_config(&config_file,
			"FileServerIP", "FileServerPort", file_server_count);
	...
	serv_info_t* db_server_list2 = new serv_info_t[db_server_count2];//有两个数据库监听地址，每个监听地址上有两个连接
	for (uint32_t i = 0; i < db_server_count2; i++) {
		db_server_list2[i].server_ip =
				db_server_list[i / concurrent_db_conn_cnt].server_ip.c_str();
		db_server_list2[i].server_port = db_server_list[i
				/ concurrent_db_conn_cnt].server_port;
	}
	...
	log("db_server_count2=%d,  concurrent_db_conn_cnt=%d.",
			db_server_count2, concurrent_db_conn_cnt);
	init_db_serv_conn(db_server_list2, db_server_count2,
