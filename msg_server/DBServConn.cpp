...
CDBServConn* get_db_serv_conn_for_login()
{
    ...
    CDBServConn* pDBConn = get_db_server_conn_in_range(0,
            g_db_server_login_count);
    if (!pDBConn)
    {
        pDBConn = get_db_server_conn_in_range(g_db_server_login_count,
                g_db_server_count);
    }
...
CDBServConn* get_db_serv_conn()
{
    ...
    CDBServConn* pDBConn = get_db_server_conn_in_range(g_db_server_login_count,
            g_db_server_count);
    if (!pDBConn)
    {
...
void CDBServConn::HandlePdu(CImPdu* pPdu)
{
    ...
    case CID_BUDDY_LIST_ADD_USER_RESPONSE:
        _HandleAddUserResponse(pPdu);
        break;

    case CID_BUDDY_LIST_AGREE_ADD_USER_REQUEST:
        _HandleAgreeAddUserResponse(pPdu);
        break;

    case CID_BUDDY_LIST_DEL_REQUEST_USER_RESPONSE:
        _HandleDelUserResponse(pPdu);
        break;

    case CID_BUDDY_LIST_ADD_REQUEST_USER_RESPONSE:
        _HandleAllUserRequestResponse(pPdu);
        break;
    case CID_BUDDY_LIST_QUERY_USER_RESPONSE:
        _HandleQueryUserRequestResponse(pPdu);
        break;
    default:
...
void CDBServConn::_HandleValidateResponse(CImPdu* pPdu)
{
    ...
    log("HandleValidateResp, user_name=%s, result=%d", login_name.c_str(),
            result);
    //通过昵称获取用户信息
    CImUser* pImUser = CImUserManager::GetInstance()->GetImUserByLoginName(
            login_name);
    ...
    if (result == 0)
    {
        ...
        { //前面存在的话删除掉前面的
            ...
            {
                CImUserManager::GetInstance()->RemoveImUserByLoginName(
                        login_name);
                delete pImUser;
            }
        } else
        ...
        pUser->KickOutSameClientType(pMsgConn->GetClientType(),
                IM::BaseDefine::KICK_REASON_DUPLICATE_USER, pMsgConn);

        CRouteServConn* pRouteConn = get_route_serv_conn();
        log("user_name  CID_OTHER_SERVER_KICK_USER111");
        if (pRouteConn)
        {
            ...
            log("user_name  CID_OTHER_SERVER_KICK_USER");
        }
...
void CDBServConn::_HandleRecentSessionResponse(CImPdu *pPdu)
{
    ...
    log("HandleRecentSessionResponse, userId=%u, session_cnt=%u", user_id,
            session_cnt);
...
void CDBServConn::_HandleAllUserResponse(CImPdu *pPdu)
{
    ...
    log("HandleAllUserResponse, userId=%u, latest_update_time=%u, user_cnt=%u",
...
void CDBServConn::_HandleAddUserResponse(CImPdu *pPdu)
{
    IM::Buddy::IMAddUserRsp msg;
    CHECK_PB_PARSE_MSG(
            msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    uint32_t user_id = msg.user_id();
    CDbAttachData attach_data((uchar_t*) msg.attach_data().c_str(),
    ...
    log("_HandleAddUserResponse, userId=%u, return code = %d", user_id,
            msg.result_code());
...
void CDBServConn::_HandleAgreeAddUserResponse(CImPdu *pPdu)
{
    IM::Buddy::IMAgreeAddUserRsp msg;
    CHECK_PB_PARSE_MSG(
            msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    ...
    log("_HandleAgreeAddUserResponse, userId=%u", user_id);
...
void CDBServConn::_HandleDelUserResponse(CImPdu *pPdu)
{
    IM::Buddy::IMDelUserRsp msg;
    log("_HandleDelUserResponse");
    CHECK_PB_PARSE_MSG(
            msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    ...
    log("_HandleDelUserResponse, userId=%u, ret = %d,", user_id,
            msg.result_code());
...
void CDBServConn::_HandleAllUserRequestResponse(CImPdu *pPdu)
{
    IM::Buddy::IMListAddRequestUserRsp msg;
    CHECK_PB_PARSE_MSG(
            msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    ...
    log(
            "_HandleAllUserRequestResponse, userId=%u, latest_update_time=%u, user_cnt=%u",
            user_id, latest_update_time, user_cnt);
...
void CDBServConn::_HandleQueryUserRequestResponse(CImPdu *pPdu)
{
    IM::Buddy::IMQueryUserRsp msg;
    CHECK_PB_PARSE_MSG(
            msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    ...
    CDbAttachData attach_data((uchar_t*) msg.attach_data().c_str(),
            msg.attach_data().length());
    uint32_t handle = attach_data.GetHandle();
    log("_HandleQueryUserRequestResponse, userId=%u", user_id);
    CMsgConn* pMsgConn = CImUserManager::GetInstance()->GetMsgConnByHandle(
            user_id, handle);
...
void CDBServConn::_HandleGetMsgListResponse(CImPdu *pPdu)
{
    ...
    log(
            "HandleGetMsgListResponse, userId=%u, session_type=%u, opposite_user_id=%u, msg_id_begin=%u, cnt=%u.",
...
void CDBServConn::_HandleGetMsgByIdResponse(CImPdu *pPdu)
{
    ...
    log(
            "HandleGetMsgByIdResponse, userId=%u, session_type=%u, opposite_user_id=%u, cnt=%u.",
...
void CDBServConn::_HandleMsgData(CImPdu *pPdu)
{
    ...
    if (msg_id == 0)
    {
        log("HandleMsgData, write db failed, %u->%u.", from_user_id,
                to_user_id);
        return;
    }
    ...
    log("HandleMsgData, from_user_id=%u, to_user_id=%u, msg_id=%u.",
    ...
    msg.clear_attach_data();
    pPdu->SetPBMsg(&msg);
    CImUser* pFromImUser = CImUserManager::GetInstance()->GetImUserById(
            from_user_id);
    CImUser* pToImUser = CImUserManager::GetInstance()->GetImUserById(
            to_user_id);
    pPdu->SetSeqNum(0);
    if (pFromImUser)
    {
        pFromImUser->BroadcastClientMsgData(pPdu, msg_id, pMsgConn,
                from_user_id);
    }
...
void CDBServConn::_HandleGetLatestMsgIDRsp(CImPdu *pPdu)
{
    ...
    log(
            "HandleUnreadMsgCntResp, userId=%u, session_id=%u, session_type=%u, latest_msg_id=%u.",
...
void CDBServConn::_HandleUnreadMsgCountResponse(CImPdu* pPdu)
{
    ...
    log("HandleUnreadMsgCntResp, userId=%u, total_cnt=%u, user_unread_cnt=%u.",
...
void CDBServConn::_HandleRemoveSessionResponse(CImPdu* pPdu)
{
    ...
    CDbAttachData attach_data((uchar_t*) msg.attach_data().c_str(),
            msg.attach_data().length());
    uint32_t handle = attach_data.GetHandle();
    CMsgConn* pConn = CImUserManager::GetInstance()->GetMsgConnByHandle(user_id,
            handle);
...
void CDBServConn::_HandleDepartmentResponse(CImPdu *pPdu)
{
    ...
    log(
            "HandleDepartmentResponse, user_id=%u, latest_update_time=%u, dept_cnt=%u.",
...
void CDBServConn::_HandleGetDeviceTokenResponse(CImPdu *pPdu)
{
    ...
    CHECK_PB_PARSE_MSG(
            msg2.ParseFromArray(msg.attach_data().c_str(),
                    msg.attach_data().length()));
            ...
            || msg_type == IM::BaseDefine::MSG_TYPE_GROUP_TEXT)
    {
        ...
        {
            log(
                    "HandleGetDeviceTokenResponse, decrypt msg failed, from_id: %u, to_id: %u, msg_type: %u.",
    ...
    for (uint32_t i = 0; i < user_token_cnt; i++)
    {
        ...
        log(
                "HandleGetDeviceTokenResponse, user_id = %u, device_token = %s, push_cnt = %u, client_type = %u.",
        ...
        if (pUser)
        {
            ...
            user_token_tmp->set_user_type(
                    (IM::BaseDefine::ClientType) client_type);
        ...
        } else
        {
            ...
            msg5.set_attach_data(attach_data.GetBuffer(),
...
void CDBServConn::_HandleChangeSignInfoResponse(CImPdu* pPdu)
{
    ...
    {
        log(
                "HandleChangeSignInfoResp: can't found msg_conn by user_id = %u, handle = %u",
                user_id, handle);
...
void CDBServConn::_HandlePushShieldResponse(CImPdu* pPdu)
{
    ...
    {
        log(
                "_HandlePushShieldResponse: can't found msg_conn by user_id = %u, handle = %u",
                user_id, handle);
    }
}
...
void CDBServConn::_HandleQueryPushShieldResponse(CImPdu* pPdu)
{
    ...
    log("_HandleQueryPushShieldResponse: user_id=%u, result=%u.", user_id,
            result);
    ...
    {
        log(
                "_HandleQueryPushShieldResponse: can't found msg_conn by user_id = %u, handle = %u",
                user_id, handle);
    }
}
