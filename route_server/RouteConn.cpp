...
void CRouteConn::HandlePdu(CImPdu* pPdu) {
	if (pPdu->GetCommandId() != CID_OTHER_HEARTBEAT)
		log("CRouteConn::HandlePdu cmd id: %d ", pPdu->GetCommandId());
	switch (pPdu->GetCommandId()) {
	case CID_OTHER_HEARTBEAT:
		// do not take any action, heart beat only update m_last_recv_tick
		break;
	case CID_OTHER_ONLINE_USER_INFO: //基本没用，在msgServer连接上的时候发送当前登录的用户
		_HandleOnlineUserInfo(pPdu);
		break;
	case CID_OTHER_USER_STATUS_UPDATE: //用户上下线通知
		_HandleUserStatusUpdate(pPdu);
		break;
	case CID_OTHER_ROLE_SET: // 没用
		_HandleRoleSet(pPdu);
		break;
	case CID_BUDDY_LIST_USERS_STATUS_REQUEST: //获取指定列表用户的状态
	...
	case CID_BUDDY_LIST_REMOVE_SESSION_NOTIFY:
		_BroadcastMsg(pPdu, this); //向连接他的Route广播消息
		break;
...
void CRouteConn::_HandleUserStatusUpdate(CImPdu* pPdu) {
	...
	log("HandleUserStatusUpdate, status=%u, uid=%u, client_type=%u ",
	...
	if (CHECK_CLIENT_TYPE_PC(client_type)) {
		...
		if (pUser) {
			...
			//此种情况一般是pc客户端多点登录时引起
			if (USER_STATUS_OFFLINE == user_status
					&& pUser->IsPCClientLogin()) {
				return;
			} else {
				_BroadcastMsg(&pdu2);
	...
	for (uint32_t i = 0; i < query_count; i++) {
		...
		if (pUser) {
			user_stat->set_status(
					(::IM::BaseDefine::UserStatType) pUser->GetStatus());
		} else {
...
void CRouteConn::_UpdateUserStatus(uint32_t user_id, uint32_t status,
	...
	if (pUser) { //找到了该用户
		if (pUser->FindRouteConn(this)) {
			if (status == USER_STATUS_OFFLINE) //离线
					{
			...
			if (status != USER_STATUS_OFFLINE) //非离线，则添加RouteConn和客户端类型
					{
	...
	{
		if (status != USER_STATUS_OFFLINE) { //非离线 添加 USerInfo和Conn以及客户端类型
...
void CRouteConn::_BroadcastMsg(CImPdu* pPdu, CRouteConn* pFromConn) {
	...
	for (it = g_route_conn_map.begin(); it != g_route_conn_map.end(); it++) {
		...
		if (pRouteConn != pFromConn) {//不再往发送该消息的地方发送
...
void CRouteConn::_SendPduToUser(uint32_t user_id, CImPdu* pPdu, bool bAll) {
	...
	if (pUser) {
		...
		for (set<CRouteConn*>::iterator it = pUserSet->begin();
				it != pUserSet->end(); it++) {
