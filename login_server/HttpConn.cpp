...
void CHttpConn::_HandleMsgServRequest(string& url, string& post_data) {
	...
	if (it_min_conn == g_msg_serv_info.end()) {
		...
		if (pIpParser->isTelcome(GetPeerIP())) {
			value2["priorIP"] = string(it_min_conn->second->ip_addr1);
			value2["backupIP"] = string(it_min_conn->second->ip_addr2);
			value2["msfsPrior"] = strMsfsUrl;
			value2["msfsBackup"] = strMsfsUrl;
		} else {
			value2["priorIP"] = string(it_min_conn->second->ip_addr2);
			value2["backupIP"] = string(it_min_conn->second->ip_addr1);
			value2["msfsPrior"] = strMsfsUrl;
			value2["msfsBackup"] = strMsfsUrl;
		}
		value2["discovery"] = strDiscovery;
		value2["port"] = int2string(it_min_conn->second->port);
		value["data"] = value2;
		string strContent = value.toStyledString();
		char* szContent = new char[HTTP_RESPONSE_HTML_MAX];
