...
void CLoginConn::Close() {
	if (m_handle != NETLIB_INVALID_HANDLE) {
		...
		if (m_conn_type == LOGIN_CONN_TYPE_CLIENT) {
			...
			map<uint32_t, msg_serv_info_t*>::iterator it = g_msg_serv_info.find(
					m_handle);
...
//消息服务器启动的时候会连接登录服务器，并把相关的消息服务器的ip port等发送到登录服务器，
//客户端连接登录服务器的时候返回给客户端
void CLoginConn::_HandleMsgServInfo(CImPdu* pPdu) {
	...
	log(
			"MsgServInfo, ip_addr1=%s, ip_addr2=%s, port=%d, max_conn_cnt=%d, cur_conn_cnt=%d, "
			"hostname: %s. ",
			pMsgServInfo->ip_addr1.c_str(), pMsgServInfo->ip_addr2.c_str(), pMsgServInfo->port, pMsgServInfo->max_conn_cnt, pMsgServInfo->cur_conn_cnt, pMsgServInfo->hostname.c_str());
}
//当前消息服务器长连接数更新
...
void CLoginConn::_HandleMsgServRequest(CImPdu* pPdu) {
	...
	map<uint32_t, msg_serv_info_t*>::iterator it_min_conn =
			g_msg_serv_info.end(), it;
